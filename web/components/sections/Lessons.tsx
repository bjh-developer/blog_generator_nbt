      ...
      <div className="mx-auto mt-12 flex max-w-3xl flex-col gap-5 px-6">
        {lessons.map((l) => (
          <div key={l.number} className="rounded-3xl border border-white/10 bg-white/[0.03] p-7">
            ...
            {l.applicable_to && (
              <button className="mt-5 inline-flex items-center gap-2 rounded-pill border border-white/20 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-white/70 transition-colors hover:bg-white/10">
                <ArrowUpRight size={14} /> {l.applicable_to}
              </button>
            )}
          </div>
        ))}
